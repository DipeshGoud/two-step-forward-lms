...
export default function AdminSchoolsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Schools List Grid - Touching Enterprise Cards */}
      <div className="bg-white border border-slate-200/90 rounded-xl grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-slate-100 overflow-hidden shadow-2xs">
        {store.schools.map((school) => {
          ...
          return (
            <div
              key={school.id}
              className="p-6 flex flex-col justify-between transition-colors hover:bg-slate-50/60"
            >
              <div>
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <Link
                        href={`/admin/schools/${school.id}`}
                        className="text-[16px] font-bold text-slate-900 hover:text-[var(--brand-primary)] transition-colors inline-block"
                      >
                        {school.name}
                      </Link>
                      <div>
                  ...
                  <div className="flex items-center gap-1.5">
                    <div className="flex items-center gap-1 text-xs text-slate-600 bg-slate-50 px-2 py-1 rounded border border-slate-200/60">
              ...
              <div className="mt-5 pt-3 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500">
                <span className="flex items-center gap-1.5">
                  ...
                  <span className="font-semibold text-slate-700">{assignedStaff}</span> Assigned Staff
                </span>

                <div className="flex items-center gap-3">
                  <span className="font-medium text-slate-600">
                    {assignedCoursesCount} Active Courses
                  </span>
                  <Link
                    href={`/admin/schools/${school.id}`}
